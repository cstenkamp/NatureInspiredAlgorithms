...
class Generator():
    """Purpose: Create the problem. No other variants needed as its needs to be comparable"""
...
#def makeLocallyOptimal(currGenome, evaluator, )
...
class Selector(): #selected by roulette-wheel 
    def __init__(self):
...
class Crossover(): #1-point cross-over, 
...
#        print("Values of the individuals", values)
        ...
        parentlist = selector.select_parents(population, values)
        children = crossover.crossover(population, parentlist)
        
#        print("CHILDREN", children)
        children = mutator.mutate(children, numMachines)
#        print("MUTATED CHILDREN", children)
        population = replacer.nextGeneration(population, children)
